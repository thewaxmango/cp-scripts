class SuffixArray:
    ...
    def __init__(self, string):
        ...
        self.arr = self.SAIS(self.str)
        self.lcp, self.rnk = self.Kasai()
        self.rmq = self.SparseTable(self.lcp)
    
    def SAIS(self, A: list[int]) -> list[int]:
        n = len(A)
        buckets = [0] * (max(A) + 2)
        for a in A:
            buckets[a + 1] += 1
        ...
        if len(LMS) > 1:
            ...
            for i in LMS2:
                ...
                prev = i
                SA[i] = j
            LMS = [LMS[i] for i in self.SAIS([SA[i] for i in LMS])]
        return induced_sort(LMS)

    def Kasai(self) -> tuple[list[int], list[int]]:
        A = self.str
        SA = self.arr
        n = len(A)
        rank = [0] * n

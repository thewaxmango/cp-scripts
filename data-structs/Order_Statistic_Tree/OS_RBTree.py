...
class Order_Statistic_Tree:
    ...
    def insert(self, v) -> None:
        ...
        while x is not self.__nil:
            ...
            y, x = x, x.left if v < x.key else x.right
    ...
    # T -> int
    # returns how many keys are smaller or equal
    def rankeq(self, v) -> int:
        x = self.__root
        total = 0
        while x is not self.__nil:
            if x.key <= v:
    ...
    def prev_val(self, v):
        return self.select(self.rank(v) - 1)
    
    # T -> T
    # returns the smallest key strictly larger
    def next_val(self, v):
        return self.select(self.rankeq(v))
